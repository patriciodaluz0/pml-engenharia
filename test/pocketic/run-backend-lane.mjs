...
export async function main(argv = process.argv.slice(2), env = process.env) {
  ...
  // assertion failure, a trap, a reject and a wrong value all leave the sidecar
  // creating instances perfectly well, so a real backend failure still exits
  // non-zero and still blocks the deployment.
  //
